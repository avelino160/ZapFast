...
import { registerBulkMessageRoutes } from "./routes-bulk-messages";
...
// Helper para obter userId (usa sessão ou fallback para demo)
function getUserId(req: any): string {
  const sessionUserId = (req.session as any).userId;
  if (sessionUserId) {
    return sessionUserId;
  }
  // Fallback para desenvolvimento/demo
  console.warn("⚠️ [AUTH] Usando usuário demo (DEFAULT_USER_ID). Configure autenticação!");
...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  // Registrar rotas de push notifications
  registerPushRoutes(app);
  
  // Registrar rotas de webhooks
  registerWebhookRoutes(app);
  
  // Registrar rotas de integrações
  registerIntegrationRoutes(app);
  
  // Registrar rotas de disparos em massa
  registerBulkMessageRoutes(app);
  
  // Registrar rotas de chatbot AI
  ...
  // User routes - REMOVIDO: Usar rota /api/auth/me de routes-auth.ts
  ...
  app.delete('/api/whatsapp/connections/:id', requireAuth, async (req, res) => {
    try {
      const { id } = req.params;
      ...
      for (const msg of messages) {
        try {
          ...
          if (triggeredFunnels.length > 0) {
            ...
            // Se nenhum funil foi acionado, tentar processar com chatbot AI
            try {
              const chatbotResponse = await chatbotService.processMessage(
                userId,
                contact.id,
                contact.name || `Contato ${cleanPhone}`,
                messageBody
              );
              
              if (chatbotResponse) {
                console.log(`🤖 [WEBHOOK] Resposta do chatbot gerada para ${cleanPhone}`);
                ...
                console.log(`✅ [WEBHOOK] Resposta do chatbot enviada`);
              }
            } catch (chatbotError) {
              console.error(`❌ [WEBHOOK] Erro ao processar com chatbot:`, chatbotError);
            }
          }
  ...
  app.post('/api/funnels/import', requireAuth, async (req, res) => {
    try {
      const userId = getUserId(req);
