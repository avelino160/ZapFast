...
export interface IStorage {
  ...
  savePushSubscription(subscription: PushSubscriptionData): Promise<void>;
  removePushSubscription(userId: string, endpoint: string): Promise<void>;
  getUserPushSubscriptions(userId: string): Promise<PushSubscriptionData[]>;
...
export class DatabaseStorage implements IStorage {
  ...
  async updateUser(id: string, updates: Partial<User>): Promise<User | undefined> {
    const [user] = await this.getDb().update(users)
  ...
  async savePushSubscription(subscription: PushSubscriptionData): Promise<void> {
    const db = this.getDb();
    
    // Check if subscription already exists (same endpoint for user)
  ...
  async removePushSubscription(userId: string, endpoint: string): Promise<void> {
    await this.getDb().delete(pushSubscriptions)
      .where(and(
  ...
  async getUserPushSubscriptions(userId: string): Promise<PushSubscriptionData[]> {
    const subs = await this.getDb().select().from(pushSubscriptions)
  ...
  async getAllUsers(): Promise<User[]> {
    return await this.getDb().select().from(users);
  }
...
export class MemStorage implements IStorage {
  ...
  private pushSubs = new Map<string, PushSubscriptionData>();

  async savePushSubscription(subscription: PushSubscriptionData): Promise<void> {
    const id = subscription.id || nanoid();
    this.pushSubs.set(id, { ...subscription, id });
  }
  ...
  async getUserPushSubscriptions(userId: string): Promise<PushSubscriptionData[]> {
    return Array.from(this.pushSubs.values()).filter(sub => sub.userId === userId);
  }
