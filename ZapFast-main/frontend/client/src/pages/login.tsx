...
export default function Login() {
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Salvar dados do usuário no localStorage para referência
      localStorage.setItem("demo_user_email", data.user.email);
      localStorage.setItem("demo_user_name", data.user.firstName);
      if (data.user.lastName) {
        localStorage.setItem("demo_user_surname", data.user.lastName);
      }
      localStorage.setItem("demo_logged_in", "true");
      localStorage.setItem("demo_login_timestamp", Date.now().toString());
  ...
  const handleRegister = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!response.ok || !data.success) {
        setRegPasswordError(data.error || 'Erro ao criar conta');
        setRegLoading(false);
        return;
      }
      ...
      setRegPasswordError('Erro ao criar conta. Tente novamente.');
      setRegLoading(false);
    }
  };
